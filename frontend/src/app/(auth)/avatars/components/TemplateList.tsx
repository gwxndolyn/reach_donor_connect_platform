  ...
  const onSelect = async (item: TemplateItem) => {
    ...
    try {
      ...
      const { error: updateErr } = await supabase
        .from("donors")
        .update({ avatar_id: json.data.id, gender: gender })
        .eq("auth_uid", uid);
  ...
  return (
    <div className="space-y-6">
      ...
      {!gender && (
        <div
          className={[
            "flex flex-col h-[100vh] items-center justify-center gap-4 transition-opacity duration-500",
        ...
        >
          ...
          <div className="flex flex-row items-center justify-center gap-4">
            <button
              ...
              className="w-[20vw] rounded-2xl border border-gray-300 bg-white px-6 py-8 text-lg font-semibold text-gray-800 shadow hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
              Female
            </button>
            <button
